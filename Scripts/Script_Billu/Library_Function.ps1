                    ...
                    if ($diff.SideIndicator -eq "=>") 
                        {
                            ...
                            $Department        = "$($diff.Département)"
                                ...
                                If (Get-ADUser -Filter {SamAccountName -eq $SamAccountName})
                                    {
                                    $samAccountName1 = "$samaccountName`1"
                                        $UserPrincipalName = (($diff.prenom.substring(0,1).tolower() + $diff.nom.ToLower()) + "1" + "@" + (Get-ADDomain).Forest)
                                         New-ADUser -Name $Name -DisplayName $DisplayName -SamAccountName $SamAccountName1 -UserPrincipalName $UserPrincipalName `
                                            -GivenName $GivenName -Surname $Surname `
                                            -Path $Path -AccountPassword (ConvertTo-SecureString -AsPlainText Azerty1* -Force) -Enabled $True `
                                            -OtherAttributes @{Company = $Company;Department = $Department} -ChangePasswordAtLogon $True 
                                         Write-Host "Activation de l'utilisateur $name SAM: $samaccountname1" -ForegroundColor Green
                                          Log -FilePath $LogFile -Content "Activation de l'utilisateur $name SAM: $samaccountname1"
...
#Get-ADUser -Filter * -Properties * | Select-Object GivenName, Surname, Company, Department, Title | Export-Csv -Path C:\Users\Administrator\Desktop\ADUsers.csv -NoTypeInformation -Encoding UTF8
    ...
    $cheminFichierCSV = "C:\users\Administrator\Desktop\ADUsers.csv"
    $cheminFichierCSV3 = "C:\Users\Administrator\Desktop\BillU3.csv"

    # Charger les données des fichiers CSV
    $fichierCSV1 = Import-Csv -Path $cheminFichierCSV -Delimiter "," -Header "Prenom" , "Nom", "société", "Département", "Service"  | Select-Object -Skip 4
               
    $fichierCSV2 = Import-Csv -Path $cheminFichierCSV3 -Delimiter "," -Header "Prenom" , "Nom", "Société", "Site",`
                "Département","Service","fonction","Manager - prénom","Manager - nom", "PC", "Date de naissance", "Téléphone fixe","Téléphone portable" `
                 | Select-Object -Skip 1

    # Comparer les fichiers et afficher les différences
    $differences = Compare-Object -ReferenceObject $fichierCSV1 -DifferenceObject $fichierCSV2 -Property  "Prenom" , "Nom" , "société", "Département" , "Service"
                    ...
                    if ($diff.SideIndicator -eq "=>") 
                        {
                            ...
                            $Company           = "$($diff.société)"
                            $Department        = "$($diff.Département)"

                            If (Get-ADUser -Filter {SamAccountName -ne $SamAccountName})
                                    {
                               
                                       if ($($diff.société) -eq "BillU")
                                            {
                                                New-ADUser -Name $Name -DisplayName $DisplayName -SamAccountName $SamAccountName -UserPrincipalName $UserPrincipalName `
                                                    -GivenName $GivenName -Surname $Surname -title $Company `
                                                    -Path $Path -AccountPassword (ConvertTo-SecureString -AsPlainText Azerty1* -Force) -Enabled $True `
                                                    -OtherAttributes @{Company = $Company;Department = $Department} -ChangePasswordAtLogon $True 
                                                    Write-Host "Activation de l'utilisateur $name SAM: $samaccountname" -ForegroundColor Green
                                                    Log -FilePath $LogFile -Content "Activation de l'utilisateur $name SAM: $samaccountname"
                                       ...
                                       Else
                                            {
                                                $path = "ou=$($diff.service),ou=$($diff.Département),ou=$($diff.société),OU=Prestataires,dc=billU,dc=lan"
                                                New-ADUser -Name $Name -DisplayName $DisplayName -SamAccountName $SamAccountName -UserPrincipalName $UserPrincipalName `
                                                    -GivenName $GivenName -Surname $Surname -title $Company `
                                                    -Path $Path -AccountPassword (ConvertTo-SecureString -AsPlainText Azerty1* -Force) -Enabled $True `
                                                    -OtherAttributes @{Company = $Company;Department = $Department} -ChangePasswordAtLogon $True 
...
Function CreateOUPrestataires
{
    # Vérifier si les OUs existent déjà
    $ouprestatairesExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouPrestataires'" 
    $ouImagyneExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouImagyne'" 
    $ouLivingtonExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouLevington'"
    $ouDlightExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouDlight'"
    $ouUbiExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouUbi'"
    
    $ouDptCommExists = try { Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch{}
    $ouDptJuridiqueExists = try { Get-ADOrganizationalUnit -Filter * -SearchBase $domainname2} catch {} 
    $ouDptLogicielExists = try {Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch {}
    $ouDptCommLightExists = try {Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch {}

    
    $ouRelationMediaExists = try {Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch {}
    $ouTestsExists = try {Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch {}
    $ouDroitsExists = Get-ADOrganizationalUnit -Filter "Name -eq '$ouDroits'" | Select-Object distinguishedname
    $ouRelationMediaLightExists = try { Get-ADOrganizationalUnit -Filter * -SearchBase $domainName2} catch {} 
    
          

    # Créer les OUs s'ils n'existent pas
    if (-not $ouPrestatairesExists) 
        {
            New-ADOrganizationalUnit -Name $ouPrestataires -Path "$domainName1"
            Write-Host "Création de l'OU OU=$ouPrestataires,DC=BillU,DC=lan" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouPrestataires,DC=BillU,DC=lan"
        } 
    else 
        {
            Write-Host "L'OU $ouPrestataires existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouPrestataires déjà éxistant"
        }

    if (-not $ouImagyneExists) 
        {
        New-ADOrganizationalUnit -Name $ouImagyne -Path "$domainName2"
        Write-Host "Création de l'OU OU=$ouImagyne,$domaineName2" -ForegroundColor Green
        Log -FilePath $LogFile -Content "Création de l'OU OU=$ouImagyne,$domaineName2"
        } 
    else 
        {
        Write-Host "L'OU $ouImagyne,$domaineName2 existe déjà." -ForegroundColor Yellow -BackgroundColor Black
           Log -FilePath $LogFile -Content "NoCréation- OU $ouImagyne déjà éxistant"
        }
    if (-not $ouLivingtonExists) 
        {
            New-ADOrganizationalUnit -Name $ouLivington -Path "$domainName2"
            Write-Host "Création de l'OU OU=$ouLivington,$domaineName2" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouLivington,$domaineName2"
        } 
    else 
        {
            Write-Host "L'OU $ouLivington existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouLivington déjà éxistant"
        }
    if (-not $ouDlightExists) 
        {    
            New-ADOrganizationalUnit -Name $ouDlight -Path "$domainName2"
            Write-Host "Création de l'OU OU=$ouDlight,$domainName2" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDlight,$domainName2"
        }     
    else 
        {
            Write-Host "L'OU $ouDlight existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouDlight déjà éxistant"
        }
    if (-not $ouUbiExists) 
        {
            New-ADOrganizationalUnit -Name $ouUbi -Path "$domainName2"
            Write-Host "Création de l'OU OU=$ouUbi,$domainName2" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouUbir,$domainName2"
        } 
    else 
        {
            Write-Host "L'OU $ouUbi existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouUbi déjà éxistant"
        }


# Création des ou services société I-magyne 
         if (-not $ouDptCommExists) 
        {
            New-ADOrganizationalUnit -Name $ouDptComm -Path "$domainName3"
            Write-Host "Création de l'OU OU=$ouDptComm,$domainName3" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDptComm,$domainName3"
            
        } 
        else 
        {
            Write-Host "L'OU $ouDptComm existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouDptComm déjà éxistant"
        }

         if (-not $ouRelationMediaExists) 
        {
            New-ADOrganizationalUnit -Name $ouRelationMedia -Path "$domainName7"
            Write-Host "Création de l'OU OU=$ouRelationMedia,$domainName7" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouRelationMedia,$domainName7"
            
        } 
        else 
        {
            Write-Host "L'OU $ouRelationMedia existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouRelationMedia déjà éxistant"
        }

# Société StudioDlight

  if (-not $ouDptCommLightExists) 
        {
            New-ADOrganizationalUnit -Name $ouDptComm -Path "$domainName5"
            Write-Host "Création de l'OU OU=$ouDptComm,$domainName5" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDptComm,$domainName5"
        ...
        else 
        {
         ...
         if (-not $ouRelationMediaLightExists) 
        {
            New-ADOrganizationalUnit -Name $ouRelationMedia -Path "$domainName8"
            Write-Host "Création de l'OU OU=$ouRelationMedia,$domainName8" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouRelationMedia,$domainName8"
...
#Société Levington
        if ( -not $ouDptJuridiqueExists) 
        {
            New-ADOrganizationalUnit -Name $ouDptJuridique -Path "$domainName4"
            Write-Host "Création de l'OU OU=$ouDptJuridique,$domainName4" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDptJuridique,$domainName4"
        } 
        else 
        {
            Write-Host "L'OU $ouDptJuridique existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouDptJuridique déjà éxistant"
        }

        
        if (-not $ouDroitsExists) 
        {
            New-ADOrganizationalUnit -Name $ouDroits -Path "$domainName10"
            Write-Host "Création de l'OU OU=$ouDroits,$domainName10" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDroits,$domainName10"
        } 
        else 
        {
            Write-Host "L'OU $ouDroits existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouDroits déjà éxistant"
        }

# Société UBIHard

         if ( -not $ouDptLogicielExists) 
        {
            New-ADOrganizationalUnit -Name $ouDptLogiciel -Path "$domainName6"
            Write-Host "Création de l'OU OU=$ouDptLogiciel,$domainName6" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouDptLogiciel,$domainName6"
        } 
        else 
        {
            Write-Host "L'OU $ouDptLogiciel existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouDptLogiciel déjà éxistant"
        }


        if ( -not $ouTestsExists) 
        {
            New-ADOrganizationalUnit -Name $ouTests -Path "$domainName9"
            Write-Host "Création de l'OU OU=$ouTests,$domainName9" -ForegroundColor Green
            Log -FilePath $LogFile -Content "Création de l'OU OU=$ouTests,$domainName9"
        } 
        else 
        {
            Write-Host "L'OU $ouTests existe déjà." -ForegroundColor Yellow -BackgroundColor Black
               Log -FilePath $LogFile -Content "NoCréation- OU $ouTests déjà éxistant"
        }
